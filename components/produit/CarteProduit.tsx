    ...
    <article className="group">
      <Link href={href} className="relative block aspect-[4/5] overflow-hidden bg-sv-warm-white">
        ...
        {produit.est_page_gamme && (
          <span className="absolute left-3 top-3 bg-sv-cream/90 px-2 py-1 text-[10px] uppercase tracking-[0.15em] text-sv-gold">
            Gamme
          </span>
        )}
        {epuise && (
      ...
      <div className="mt-3">
        <div className="flex items-baseline justify-between gap-2">
          <Link href={href}>
            <h3 className="font-serif text-lg leading-tight transition-colors group-hover:text-sv-gold">{produit.nom}</h3>
          </Link>
          <p className="shrink-0 text-sm">
            {enPromo && <span className="mr-1 text-sv-mid line-through">{formatPrix(v.prix)}</span>}
            <span className={enPromo ? 'text-sv-gold' : ''}>{formatPrix(prixActuel)}</span>
          </p>
        </div>
